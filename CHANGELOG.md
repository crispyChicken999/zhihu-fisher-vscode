...
- 收藏夹列表采用惰性加载方式，避免多次调用接口导致Cookie失效的问题。提供刷新的功能，方便用户在有需要的时候进行刷新。
...
- 优化了插件关闭时候的cleanUp方法，进一步减少了内存泄漏的可能性。
- 修复了沉浸模式按钮提示被遮挡的问题。
- 优化了内容加载逻辑，点击回答则优先加载回答的内容而不是直接开始加载全部内容。确保了内容与侧边栏tooltip一致性。
- 优化了webviewID的生成逻辑，同一个问题不同回答现已能够同时开启，提升了用户的使用体验。
