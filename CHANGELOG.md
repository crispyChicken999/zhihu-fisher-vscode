...
- 修复了 Puppeteer 在系统的temp目录下创建大量临时文件的问题，现已改为使用用户数据目录（os.tmpdir()/zhihu-fisher-puppeteer-dev-chrome-profile），意思是使用同一个目录存储浏览器的用户数据，那么后续启动浏览器就会使用这个目录进行加载，避免每次启动都创建新的用户数据目录，从而减少临时文件的数量。
