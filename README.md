...
3. 支持 加载搜索结果
4. 支持 加载特定知乎链接的内容
5. 支持 支持对内容进行赞同/不赞同的操作
6. 支持 完整的收藏夹功能，查看和管理自己创建和关注的收藏夹
7. 支持 切换多媒体（图片、视频等）模式（隐藏|迷你|正常）的功能，偷摸摸or光明正大地看图片😏
8. 支持 阅读区样式调整，详情页可以调整字体大小、颜色、排版等样式，更加隐蔽地摸鱼
9. 支持 快捷键操作，键盘 `← →` 切换上/下一个回答，按`/`可以切换图片显示，按 `Crtl+↑↓ / W|S` 切换上/下篇内容
10. 支持 评论区，可以查看文章/回答的评论内容，同时支持查看评论的子评论
11. 支持 沉浸模式（推荐使用 👍），隐藏其他不重要的信息，只显示问题标题和回答内容
12. 支持 收藏文章、问答到指定收藏夹，支持缓存收藏夹信息，避免每次都要重新加载
13. 支持 灰色模式，将页面所有内容转为灰色显示，进一步地摸鱼，降低被老板发现的风险 😂
14. 支持 智能伪装模式，切到其他页面后自动将文章页面的标题和图标伪装为代码文件，避免摸鱼被发现，切回后自动恢复
15. 支持 智能伪装模式自定义，用户可以自定义伪装的标题和图标，满足不同用户的需求
...
2.  为啥要等推荐列表加载出来才能查看内容呢？因为知乎首页很鸡贼，如果在加载过程中，你查看了内容，那么 Puppeteer 就会打开一个新标签页获取内容，那么首页的标签页失去焦点，滚动加载更多就失效了，所以需要等推荐列表加载完成后再查看内容。
    > 我是想着不管你机制怎么变，我都是模拟人在访问，那么就算慢点也无妨，毕竟也能够加载出来 😂
3.  因为新版接口有防爬机制，所以用的是无限制的旧版接口，那么有时候可能会看到重复的评论，这个我也没招，毕竟是接口返回回来的。哎凑合着看吧哈哈...
4.  在加载回答的时候，会出现页面状态重置的问题。
    1.  就比如打开了子评论的弹窗正看着，然后又加载了一批回答，那么页面就会回到初始状态（对应的就是子评论弹窗被关闭，页面回到顶部）。
    2.  这个问题是因为直接将整个 html 文档替换来更新状态导致的。代码是用 ts 写的没用主流框架，暂时也没动力改了，凑合着用吧。
    3.  （其实应该通过 postMessage 的方式去通知文档哪里需要更新，而不是整个文档重新生成然后替换）
5.  其他问题，欢迎在 [Issues](https://github.com/crispyChicken999/zhihu-fisher-vscode/issues) 中进行反馈，我看到后会尽快修复。
